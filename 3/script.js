...
function createEtherpadID() {
    $.ajax({
        ...
        success: function(data) {
            document.getElementById('etherpad-lite').src = 'http://localhost:9001/p/' + getWorkerID() + '?showControls=true&showChat=false&showLineNumbers=true&useMonospaceFont=false';
        }
    });
...
function saveHistory() {
    var stringified = JSON.stringify(historyArray);
    console.log(stringified);
    $.ajax({
        data: {"history" : stringified},
        url: 'submit.php',
        type: 'POST',
       ...
       // dataType: 'json',
        ...
        error: function(req, stat, err) {
            console.log('something went wrong', stat, err);
        }
    });
}
